		...
		foreach (string asset in importedAssets) {
			...
			using (FileStream stream = File.Open (filePath, FileMode.Open, FileAccess.Read)) {
				...
				foreach(string sheetName in sheetNames) {
					...
					cell = row.GetCell(4); p.Prefab = (cell == null ? "" : cell.StringCellValue);
					cell = row.GetCell(5); p.SearchRange = (int)(cell == null ? 0 : cell.NumericCellValue);
					cell = row.GetCell(6); p.AttackPower = (int)(cell == null ? 0 : cell.NumericCellValue);
					cell = row.GetCell(7); p.AttackSpeed = (float)(cell == null ? 0 : cell.NumericCellValue);
						s.list.Add (p);
					}
					data.sheets.Add(s);
				}
