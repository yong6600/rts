    ...
    public class Agent : IAgent
    {
        ...
        public void CalculateNeighbours()
        {
            ...
            if (MaxNeighbours > 0)
            {
                ...
                //!edit
                simulator.Quadtree.Query(new Vector2(position.x, position.y), neighbourDist, this);
            }
            //watch1.Stop ();
        ...
        public float InsertAgentNeighbour(Agent agent, float rangeSq)
        {
            ...
            float dist = Sqr(agent.position.x - position.x) + Sqr(agent.position.y - position.y);
        ...
        {
            return new Vector3(p.x, p.y, 0f); //return new Vector3(p.x, 0, p.y);
        }
        ...
        internal void CalculateVelocity(Pathfinding.RVO.Simulator.WorkerContext context)
        {
            ...
            //edit
            Vector2 optimalVelocity = new Vector2(velocity.x, velocity.y);
            ...
            for (int i = 0; i < simulator.obstacles.Count; i++)
            {
                ...
                do
                {
                    ...
                    float cross = VO.Det(new Vector2(vertex.position.x, vertex.position.y), vertex.dir, position2D);// vertex.dir.x * ( vertex.position.z - position.z ) - vertex.dir.y * ( vertex.position.x - position.x );
                    ...
                    float dotFactor = Vector2.Dot(vertex.dir, position2D - new Vector2(vertex.position.x, vertex.position.y));
                    ...
                    bool closestIsEndpoints = dotFactor <= wallThickness * 0.05f || dotFactor >= (new Vector2(vertex.position.x, vertex.position.y) - new Vector2(vertex.next.position.x, vertex.next.position.y)).magnitude - wallThickness * 0.05f;
                    ...
                    if (Mathf.Abs(signedDist) < neighbourDist)
                    {
                        if (signedDist <= 0 && !closestIsEndpoints && signedDist > -wallThickness)
                        {
                            ...
                            vos[voCount] = new VO(position2D, new Vector2(vertex.position.x, vertex.position.y) - position2D, vertex.dir, wallWeight * 2);
                            voCount++;
                        }
                        else if (signedDist > 0)
                        {
                            ...
                            Vector2 p1 = new Vector2(vertex.position.x, vertex.position.y) - position2D;
                            Vector2 p2 = new Vector2(vertex.next.position.x, vertex.next.position.y) - position2D;
            ...
            for (int o = 0; o < neighbours.Count; o++)
            {
                ...
                Vector2 otherOptimalVelocity = new Vector2(other.Velocity.x, other.velocity.y);
                ...
                Vector2 voBoundingOrigin = new Vector2(other.position.x, other.position.y) - position2D;
            ...
            if (simulator.algorithm == Simulator.SamplingAlgorithm.GradientDescent)
            {
                if (DebugDraw)
                {
                    ...
                    for (int x = 0; x < PlotWidth; x++)
                    {
                        for (int y = 0; y < PlotWidth; y++)
                        {
                            ...
                            Vector2 d2 = (new Vector2(desiredVelocity.x, desiredVelocity.y) - (p - position2D));
                ...
                float cutoff = new Vector2(velocity.x, velocity.y).magnitude * simulator.qualityCutoff;
            ...
            else
            {
                ...
                Vector2 desired2D = new Vector2(desiredVelocity.x, desiredVelocity.y);
        ...
        Vector2 Trace(VO[] vos, int voCount, Vector2 p, float cutoff, out float score)
        {
            ...
            for (int s = 0; s < 50; s++)
            {
                ...
                Vector2 bonusForDesiredVelocity = (new Vector2(desiredVelocity.x, desiredVelocity.y) - p);
